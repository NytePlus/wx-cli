...
fn ffmpeg_bin() -> String {
    if let Some(path) = bundled_binary("imcp-ffmpeg") {
        return path;
    }
...
fn ffprobe_bin() -> String {
    if let Some(path) = bundled_binary("imcp-ffprobe") {
        return path;
    }
...
fn bundled_binary(name: &str) -> Option<String> {
    ...
    if exe.file_name()?.to_str()? != "imcp-wechat" {
        return None;
    }
    ...
    std::thread::scope(|scope| {
        ...
        if timed_out || stdout.len() > 100 * 1024 * 1024 || stderr.len() > 65536 {
            return Err(MediaError::FfmpegFailed {
                status: -1,
                stderr: "transcode resource limit exceeded".into(),
            });
        }
        Ok(Output {
            status,
